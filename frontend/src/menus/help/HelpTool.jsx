  ...
  const tools = [
    {
      ...
      usage: [
        ...
        '  - 使用鼠标中键点击标签页也可以快速关闭',
        '  - 右键标签页可固定/取消固定，固定标签会排序到最前',
        '  - 标签页支持拖拽排序（固定区与未固定区分别组内拖拽）',
    ...
    },
    {
      ...
      usage: [
        '输入时间戳（秒或毫秒）可转换为日期时间',
        '或选择日期时间转换为时间戳',
        '毫秒时间戳转换为时间时，DateTime 格式会自动带上毫秒位（如 2026-08-10 20:30:26.123）',
        '时间转时间戳同时输出秒级与毫秒级，均可复制',
        '支持多种时间格式（默认 YYYY-MM-DD HH:mm:ss）',
        '时间戳转时间自动识别单位，仅支持 10 位（秒）或 13 位（毫秒）',
        '点击「当前」可填入当前秒级时间戳',
        '转换历史会记录最近 50 条成功记录，重启应用后仍会保留并存储在本地数据库',
        '点击标题栏右上角"历史记录"按钮可打开/关闭右侧历史面板',
        '历史面板支持每条记录"复制输出"，并可点击记录快速回填输入',
        '历史面板支持一键重置（清空历史与当前转换数据）',
        '鼠标点击历史面板外区域会自动关闭面板，也可通过面板内关闭按钮独立关闭'
      ]
    },
    {
      icon: '🆔',
